...
ScalarField initialize_scalar_field(const size_t size, const double initial_value) noexcept {
  ScalarField field{size};
  std::iota(field.x.begin(), field.x.end(), initial_value);
  return field;
}

ScalarField scale_scalar_field(const ScalarField& field, const double scale_factor) noexcept {
...
// && means an R-value reference
ScalarField scale_scalar_field(ScalarField&& field, const double scale_factor) noexcept {
...
int main() {
  ...
  std::cout << "Initializing a scalar field of size " << size << "\n";
  ...
  std::cout << "Now scaling with std::move " << scale_factor << "\n";
  start_time = clock::now();
